...
public class FullBlockTestGenerator {
    ...
    public RuleList getBlocksToTest(boolean addSigExpensiveBlocks, boolean runLargeReorgs, File blockStorageFile) throws ScriptException, ProtocolException, IOException {
        ...
        final Coin fiftyCoins = Coin.fiftyCoins(params.getCoinDefinition());
        ...
        spendableOutputs.offer(new TransactionOutPointWithValue(
                ...
                fiftyCoins, chainHead.getTransactions().get(0).getOutputs().get(0).getScriptPubKey()));
        ...
        final Coin satoshi = Coin.satoshi(params.getCoinDefinition());
        ...
        Block b15 = createNextBlock(b13, chainHeadHeight + 6, out5, null);
        {
            ...
            byte[] outputScript = new byte[params.maxBlockSigops - sigOps];
        ...
        Block b16 = createNextBlock(b15, chainHeadHeight + 7, out6, null);
        {
            ...
            Transaction tx = new Transaction(params);
            byte[] outputScript = new byte[params.maxBlockSigops - sigOps + 1];
            Arrays.fill(outputScript, (byte) OP_CHECKSIG);
            tx.addOutput(new TransactionOutput(params, tx, satoshi, outputScript));
            addOnlyInputToTransaction(tx, new TransactionOutPointWithValue(
        ...
        Block b23 = createNextBlock(b15, chainHeadHeight + 7, out6, null);
        {
            ...
            byte[] outputScript = new byte[params.maxBlockSize - b23.getMessageSize() - 138];
        ...
        Block b24 = createNextBlock(b15, chainHeadHeight + 7, out6, null);
        {
            Transaction tx = new Transaction(params);
            // Signature size is non-deterministic, so it may take several runs before finding any off-by-one errors
            byte[] outputScript = new byte[params.maxBlockSize - b24.getMessageSize() - 135];
            Arrays.fill(outputScript, (byte) OP_FALSE);
            tx.addOutput(new TransactionOutput(params, tx, satoshi, outputScript));
            addOnlyInputToTransaction(tx, new TransactionOutPointWithValue(
        ...
        Block b31 = createNextBlock(b30, chainHeadHeight + 9, out8, null);
        {
            ...
            Transaction tx = new Transaction(params);
            byte[] outputScript = new byte[(params.maxBlockSigops - sigOps)/20];
        ...
        Block b32 = createNextBlock(b31, chainHeadHeight + 10, out9, null);
        {
            ...
            Transaction tx = new Transaction(params);
            byte[] outputScript = new byte[(params.maxBlockSigops - sigOps)/20 + (params.maxBlockSigops - sigOps)%20 + 1];
            Arrays.fill(outputScript, (byte) OP_CHECKMULTISIG);
            for (int i = 0; i < (params.maxBlockSigops - sigOps)%20; i++)
        ...
        final Coin zero = Coin.zero(params.getCoinDefinition());
        ...
        Block b39 = createNextBlock(b35, chainHeadHeight + 12, null, null);
        {
            ...
            while (b39.getMessageSize() < params.maxBlockSize)
            {
                Transaction tx = new Transaction(params);
                ...
                if (b39.getMessageSize() + tx.getMessageSize() < params.maxBlockSize) {
                    b39.addTransaction(tx);
                    b39numP2SHOutputs++;
                } else
                    break;
        ...
        Block b40 = createNextBlock(b39, chainHeadHeight + 13, out12, null);
        {
            ...
            int numTxes = (params.maxBlockSigops - sigOps) / b39sigOpsPerOutput;
            ...
            Transaction tx = new Transaction(params);
            tx.addInput(new TransactionInput(params, tx, new byte[]{OP_1}, lastOutPoint));
            byte[] scriptPubKey = new byte[params.maxBlockSigops - sigOps + 1];
        ...
        if (addSigExpensiveBlocks) {
            b41 = createNextBlock(b39, chainHeadHeight + 13, out12, null);
            {
                ...
                int numTxes = (params.maxBlockSigops - sigOps)
                        / b39sigOpsPerOutput;
                checkState(numTxes <= b39numP2SHOutputs);
                ...
                tx.addInput(new TransactionInput(params, tx,
                        new byte[] {OP_1}, lastOutPoint));
                byte[] scriptPubKey = new byte[params.maxBlockSigops - sigOps];
                Arrays.fill(scriptPubKey, (byte) OP_CHECKSIG);
                tx.addOutput(new TransactionOutput(params, tx, zero, scriptPubKey));
                b41.addTransaction(tx);
            }
        ...
        Block b47 = createNextBlock(b44, chainHeadHeight + 16, out15, null);
        {
            try {
                ...
                while (true) {
                    BigInteger h = b47.getPofHash().toBigInteger();
                    if (h.compareTo(target) > 0) // if invalid
                        break;
                    // increment the nonce and try again.
                    final long nn = b47.getNonce() + 1;
                    b47.setNonce(nn);
                }
        ...
        b48.setTime(Utils.currentTimeSeconds() + 60 * 60 * 3 * 10000);
        b48.solve();
        ...
        Block b50 = createNextBlock(b44, chainHeadHeight + 16, out15, null);
        {
            ...
            diffTarget = (diffTarget & 0xfffeffff) + 0x0000FFFFL; // Make difficulty one bit harder
        ...
        Block b64;
        {
            ...
            Transaction tx = new Transaction(params);
            // Signature size is non-deterministic, so it may take several runs before finding any off-by-one errors
            byte[] outputScript = new byte[params.maxBlockSize - b64Created.getMessageSize() - 138];
            Arrays.fill(outputScript, (byte) OP_FALSE);
            tx.addOutput(new TransactionOutput(params, tx, satoshi, outputScript));
            addOnlyInputToTransaction(tx, new TransactionOutPointWithValue(
        ...
        Block b73 = createNextBlock(b72, chainHeadHeight + 23, out22, null);
        {
            ...
            Transaction tx = new Transaction(params);
            byte[] outputScript = new byte[params.maxBlockSigops - sigOps + (int)Script.MAX_SCRIPT_ELEMENT_SIZE + 1 + 5 + 1];
            Arrays.fill(outputScript, (byte) OP_CHECKSIG);
            // If we push an element that is too large, the CHECKSIGs after that push are still counted
            outputScript[params.maxBlockSigops - sigOps] = OP_PUSHDATA4;
            Utils.uint32ToByteArrayLE(Script.MAX_SCRIPT_ELEMENT_SIZE + 1, outputScript, params.maxBlockSigops - sigOps + 1);
            tx.addOutput(new TransactionOutput(params, tx, satoshi, outputScript));
            addOnlyInputToTransaction(tx, new TransactionOutPointWithValue(
        ...
        Block b74 = createNextBlock(b72, chainHeadHeight + 23, out22, null);
        {
            ...
            Transaction tx = new Transaction(params);
            byte[] outputScript = new byte[params.maxBlockSigops - sigOps + (int)Script.MAX_SCRIPT_ELEMENT_SIZE + 42];
            Arrays.fill(outputScript, (byte) OP_CHECKSIG);
            // If we push an invalid element, all previous CHECKSIGs are counted
            outputScript[params.maxBlockSigops - sigOps + 1] = OP_PUSHDATA4;
            outputScript[params.maxBlockSigops - sigOps + 2] = (byte)0xfe;
            outputScript[params.maxBlockSigops - sigOps + 3] = (byte)0xff;
            outputScript[params.maxBlockSigops - sigOps + 4] = (byte)0xff;
            outputScript[params.maxBlockSigops - sigOps + 5] = (byte)0xff;
            tx.addOutput(new TransactionOutput(params, tx, satoshi, outputScript));
            addOnlyInputToTransaction(tx, new TransactionOutPointWithValue(
        ...
        Block b75 = createNextBlock(b72, chainHeadHeight + 23, out22, null);
        {
            ...
            outputScript[params.maxBlockSigops - sigOps] = OP_PUSHDATA4;
            outputScript[params.maxBlockSigops - sigOps + 1] = (byte)0xff;
            outputScript[params.maxBlockSigops - sigOps + 2] = (byte)0xff;
            outputScript[params.maxBlockSigops - sigOps + 3] = (byte)0xff;
            outputScript[params.maxBlockSigops - sigOps + 4] = (byte)0xff;
            tx.addOutput(new TransactionOutput(params, tx, satoshi, outputScript));
            addOnlyInputToTransaction(tx, new TransactionOutPointWithValue(
        ...
        Block b76 = createNextBlock(b75, chainHeadHeight + 24, out23, null);
        {
            ...
            Transaction tx = new Transaction(params);
            byte[] outputScript = new byte[params.maxBlockSigops - sigOps + (int)Script.MAX_SCRIPT_ELEMENT_SIZE + 1 + 5];
            Arrays.fill(outputScript, (byte) OP_CHECKSIG);
            // If we push an element that is filled with CHECKSIGs, they (obviously) arent counted
            outputScript[params.maxBlockSigops - sigOps] = OP_PUSHDATA4;
            Utils.uint32ToByteArrayLE(params.maxBlockSigops, outputScript, params.maxBlockSigops - sigOps + 1);
            tx.addOutput(new TransactionOutput(params, tx, satoshi, outputScript));
            addOnlyInputToTransaction(tx, new TransactionOutPointWithValue(
        ...
        post82Mempool.add(InventoryItem.createTransactionItem(b78tx.getHash()));
        post82Mempool.add(InventoryItem.createTransactionItem(b79tx.getHash()));
        ...
        if (runLargeReorgs) {
            ...
            for (blockCountAfter1001 = 0; blockCountAfter1001 < TRANSACTION_CREATION_BLOCKS; blockCountAfter1001++) {
                ...
                while (block.getMessageSize() < params.maxBlockSize - 500) {
                    Transaction tx = new Transaction(params);
            ...
            for (int i = 0; hashes.hasNext(); i++) {
                Block block = createNextBlock(lastBlock, nextHeight++, null, null);
                while (block.getMessageSize() < params.maxBlockSize - 500 && hashes.hasNext()) {
                    Transaction tx = new Transaction(params);
    ...
    private Block createNextBlock(Block baseBlock, int nextBlockHeight, @Nullable TransactionOutPointWithValue prevOut,
        ...
        Coin coinbaseValue = Coin.fiftyCoins(params.getCoinDefinition()).shiftRight(nextBlockHeight / params.getSubsidyDecreaseBlockCount())
