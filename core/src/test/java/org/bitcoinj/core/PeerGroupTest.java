...
public class PeerGroupTest extends TestWithPeerGroup {
    ...
    public void receiveTxBroadcastOnAddedWallet() throws Exception {
        ...
        final Message outbound1 = outbound(p1);
        assertTrue(String.valueOf(outbound1), outbound1 instanceof BloomFilter);
        final Message outbound2 = outbound(p1);
        assertTrue(String.valueOf(outbound2), outbound2 instanceof MemoryPoolMessage);

        Coin value = coin(unitTestParams);
    ...
    public void downloadPeerSelection() throws Exception {
        ...
        versionMessage2.clientVersion = params.getCoinDefinition().getMinBloomProtocolVersion();
        versionMessage2.localServices = 1;
        VersionMessage versionMessage3 = new VersionMessage(params, 3);
        versionMessage3.clientVersion = params.getCoinDefinition().getMinBloomProtocolVersion();
        versionMessage3.localServices = 1;
    ...
    public void preferLocalPeer() throws IOException {
        // Because we are using the same port that is used by litecoin client
        ...
        // Check that if we have a localhost port of litecoin client then it's used instead of the p2p network.
        ServerSocket local;
        try {
    ...
    public void autoRescanOnKeyExhaustion() throws Exception {
        ...
        Coin expectedBalance = Coin.zero(params.getCoinDefinition());
        Block prev = blockStore.getChainHead().getHeader();
        final Coin fiftyCoins = Coin.fiftyCoins(unitTestParams.getCoinDefinition());
        for (ECKey key1 : keys) {
            Address addr = key1.toAddress(params);
            Block next = FakeTxBuilder.makeSolvedTestBlock(prev, FakeTxBuilder.createFakeTx(params, fiftyCoins, addr));
        ...
        assertEquals(fiftyCoins.multiply(3), wallet.getBalance());
        ...
        assertEquals(InventoryItem.TYPE_FILTERED_BLOCK, getdata.getItems().get(0).type);
    ...
    public void pings() throws Exception {
        ...
        versionMessage.clientVersion = params.getCoinDefinition().getMinBloomProtocolVersion();
    ...
    public void testBloomOnP2Pubkey() throws Exception {
        ...
        Transaction tx = FakeTxBuilder.createFakeTx(params, coin(unitTestParams.getCoinDefinition()), key);
        ...
        final InventoryItem inv = InventoryItem.createTransactionItem(dephash);
    ...
    private <T extends Message> T assertNextMessageIs(InboundMessageQueuer q, Class<T> c) throws Exception {
        Message outbound = waitForOutbound(q);
        assertEquals(c, outbound.getClass());
